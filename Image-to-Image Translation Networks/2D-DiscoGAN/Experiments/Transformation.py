...
from Models.architectures.discogan import build_discriminator, build_generator
...
class Experiment_transformation():
    ...
    def prepare(self):
        ...
        # Get the generators
        gen_A = build_generator(self.args['input_shape'], name="discogan_generator_A")
        gen_B = build_generator(self.args['input_shape'], name="discogan_generator_B")
        
        # Get the discriminators
        disc_A = build_discriminator(self.args['input_shape'], name="discogan_discriminator_A")
        disc_B = build_discriminator(self.args['input_shape'], name="discogan_discriminator_B")

        if os.path.exists(self.args['last_models_dir'] + os.sep + 'Generator_A.h5'):
            print('[INFO] Loading pretrained model weights from: ' + self.args['last_models_dir'])

            pretrained_generator_A_path = os.path.join(self.args['last_models_dir'], 'Generator_A.h5')
            pretrained_generator_B_path = os.path.join(self.args['last_models_dir'], 'Generator_B.h5')
            pretrained_discriminator_A_path = os.path.join(self.args['last_models_dir'], 'Discriminator_A.h5')
            pretrained_discriminator_B_path = os.path.join(self.args['last_models_dir'], 'Discriminator_B.h5')
            
            gen_A.load_weights(pretrained_generator_A_path)
            gen_B.load_weights(pretrained_generator_B_path)
            disc_A.load_weights(pretrained_discriminator_A_path)
            disc_B.load_weights(pretrained_discriminator_B_path)
        ...
        if self.network_structure == 'DiscoGan':
            self.model = DiscoGAN(generator_A=gen_A, generator_B=gen_B, discriminator_A=disc_A, discriminator_B=disc_B)
    ...
    def generate_output_3D(self):
        ...
        self.model.gen_B.load_weights(os.path.join(self.args['last_models_dir'], 'Generator_B.h5'))
